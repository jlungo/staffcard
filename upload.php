...
if (isset($_POST['addleave'])) {
	if ($_POST['emp_id'] != '' && $_POST['emp_fname'] != '' && $_POST['emp_sname'] != '' && $_POST['emp_sdate'] != '' && $_POST['emp_edate'] != ''  && $_POST['emp_dep'] != '' && $_POST['emp_addr'] != '') {

		$emp_id = mysqli_real_escape_string($db, $_POST['emp_id']);
		$emp_fname = mysqli_real_escape_string($db, $_POST['emp_fname']);
		$emp_sname = mysqli_real_escape_string($db, $_POST['emp_sname']);
		$emp_sdate = mysqli_real_escape_string($db, $_POST['emp_sdate']);
		$emp_edate = mysqli_real_escape_string($db, $_POST['emp_edate']);
		$emp_addr = mysqli_real_escape_string($db, $_POST['emp_addr']);
		$emp_dep = mysqli_real_escape_string($db, $_POST['emp_dep']);
		$emp_days = mysqli_real_escape_string($db, strval(round(days_between($emp_sdate, $emp_edate))));
		$pagex = mysqli_real_escape_string($db, $_POST['page']);
		...
		$emp_name = $emp_title . " " . $emp_fname . " " . $emp_sname;

		$check = "SELECT * FROM leaves WHERE Empno='$emp_id'";
		$checks = mysqli_query($db, $check);
		$found = mysqli_num_rows($checks);
		if ($found == 0) {

			$query = "INSERT INTO leaves (EmpNo,Empname,leavestart,leaveend,totaldays,homeaddress,nameofdependant) " .
				"VALUES ('$emp_id','$emp_name', '$emp_sdate','$emp_edate','$emp_days','$emp_addr','$emp_dep')";
			$db->query($query) or die('Error1, query failed');

			$leaveadd = "tyy";
			$_SESSION['leaveadd'] = $leaveadd;
			header("Location:$pagex");  //leave added successfully



		} else {
			$_SESSION['leaveused'] = "staff already registered leave";
			header("Location:$pagex");
		}
	} else {
		$_SESSION['emptytextboxes'] = "Not all text boxes were completed";
