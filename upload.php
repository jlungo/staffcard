...
if (isset($_POST['addDependant'])) {
	if ($_POST['EmployeeNo'] != '' && $_POST['EmployeeName'] != '' && $_POST['FDepNo'] != '' && $_POST['MDepNo'] != '' && $_POST['AgeOldDep'] != '' && $_POST['AgeYoungDep'] != '' && $_POST['Address'] != '') {

		$EmployeeNo = mysqli_real_escape_string($db, $_POST['EmployeeNo']);
		$EmployeeName = mysqli_real_escape_string($db, $_POST['EmployeeName']);

		$FDepNo = mysqli_real_escape_string($db, $_POST['FDepNo']);
		$MDepNo = mysqli_real_escape_string($db, $_POST['MDepNo']);

		$AgeOldDep = mysqli_real_escape_string($db, $_POST['AgeOldDep']);
		$AgeYoungDep = mysqli_real_escape_string($db, $_POST['AgeYoungDep']);

		$Address = mysqli_real_escape_string($db, $_POST["Address"]);


		$check = "SELECT * FROM StaffDep WHERE EmployeeNo='$EmployeeNo' && EmployeeName='$EmployeeName'";
		...
		if ($found == 0) {
			...
			$query = "INSERT INTO StaffDep (EmployeeNo,EmployeeName,FDepNo,MDepNo,AgeOldDep,`AgeYoungDep`,`Address`) " .
				"VALUES ('$EmployeeNo','$EmployeeName', '$FDepNo','$MDepNo','$AgeOldDep','$AgeYoungDep','$Address')";
			$db->query($query) or die('Error1, query failed ' . $db->error);
